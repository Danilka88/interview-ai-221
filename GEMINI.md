...
- Проведение собеседований в нескольких режимах (голосовой чат, текстовая симуляция).
...
- **Бэкенд:** Python, FastAPI, Uvicorn, Pydantic, **SQLAlchemy, SQLite**
- **Frontend:** Ванильный HTML/CSS/JavaScript с использованием CSS-фреймворка Pico.css.
- **Взаимодействие с ИИ:** LangChain для управления цепочками вызовов и промптами.
- **Языковые модели (LLM):** Ollama для локального запуска моделей.
...
1.  **Пользователь** открывает `/rank`, загружает вакансию и резюме.
...
4.  **Frontend** получает JSON, сохраняет в `localStorage` и перенаправляет на `/rank/result` для отрисовки.

### Сценарий 2: Проведение собеседования (через веб-интерфейс)

1.  **Пользователь** открывает `/voice-interview` (голосовое) или `/test` (текстовое), опционально загружает файлы (вакансия, резюме).
...
5.  По завершении **фронтенд** сохраняет результат в `localStorage` и переходит на `/result` для финального анализа.
...
- Убедитесь, что у вас **запущен локальный сервер Ollama**.
