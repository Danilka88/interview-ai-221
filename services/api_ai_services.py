...
async def run_stress_test_simulation(vacancy_text: str, resume_text: str) -> dict:
    """
    Проводит полную симуляцию СТРЕСС-ТЕСТ интервью и возвращает его анализ.
    """
    logging.info("[API AI Service] Запуск полной симуляции СТРЕСС-ТЕСТ интервью...")
    ...
    candidate_prompt_template = (
        STRESS_CANDIDATE_SYSTEM_PROMPT.format(resume_text=resume_text) +
        "\n\nТекущий диалог:\n{chat_history}\n\nAI-Рекрутер: {human_input}\nКандидат:"
    )
